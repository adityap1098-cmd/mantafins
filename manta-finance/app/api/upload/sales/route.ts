...
export async function POST(request: NextRequest) {
  ...
  try {
  // Load master products for name matching (global, not per-period)
  const masterProducts = await prisma.product.findMany();
  // Build lookup map: lowercase name → product
  const snapshotMap = new Map(
    masterProducts.map((s) => [s.name.toLowerCase().trim(), s])
  );
  ...
  // Must delete SaleItems first to avoid foreign key constraint violation
  ...
  if (existingSaleIds.length > 0) {
    await prisma.saleItem.deleteMany({
      where: { saleId: { in: existingSaleIds } },
    });
