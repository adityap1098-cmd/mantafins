...
function discountColor(pct: number): string {
  if (pct <= 5) return "text-success font-medium";
...
export default function CustomerDiscountTable({
  rows,
}: CustomerDiscountTableProps) {
            ...
            <tbody>
              {sorted.map((row) => (
                ...
                >
                  ...
                  <td className="px-4 py-3 text-right text-foreground">
                    {idr.format(row.totalPenjualan)}
                  </td>
                  <td className="px-4 py-3 text-right text-foreground">
                    {idr.format(row.totalDiskon)}
                  </td>
                  <td className={`px-4 py-3 text-right ${discountColor(row.avgDiskonPersen)}`}>
